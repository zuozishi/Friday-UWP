...
namespace Friday_XMPP
{
    ...
    public sealed partial class App : Application
    {
        ...
        protected override void OnActivated(IActivatedEventArgs args)
        {
            ...
            {
                var protoarg = (ProtocolActivatedEventArgs)args;
                var url= protoarg.Uri.ToString().Replace("fridayxmpp:?","");
                var datas = url.Split('&');
                var datadir = new Dictionary<string, string>();
                foreach (var item in datas)
                {
                    datadir.Add(item.Split('=')[0], item.Split('=')[1]);
                }
                ...
                rootFrame.Navigate(typeof(MainPage), datadir);
            }
        }
